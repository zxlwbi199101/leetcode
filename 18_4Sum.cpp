...
public:
  vector<vector<int>> fourSum(vector<int>& nums, int target) {
    ...
    vector<int> sums = {0};
    if (nums.size() < COUNTS) return result;
    ...
    for (int j = 0; j < nums.size(); j++) {
      ...
      if (j && nums[j - 1] == nums[j]) {
        consecutiveEnd = consecutiveStart;
      }

      for (int i = consecutiveEnd; i < len; i++) {
        int sum = sums[i] + n;

        if (cache[i].size() < COUNTS - 1) {
          sums.push_back(sum);
          cache.push_back(cache[i]);
          cache.back().push_back(n);
        } else if (sum == target) {
...
int main() {
  ...
  vector<vector<int>> result = solution.fourSum(nums, -236727523);
