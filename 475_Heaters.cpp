...
public:
  int findRadius(vector<int>& houses, vector<int>& heaters) {
    ...
    for (int hoI = 0, heI = 0; hoI < houses.size(); hoI++) {
      while (houses[hoI] >= heaters[heI] && heI < heaters.size() - 1) {
        heI++;
      }

      int distance = abs(heaters[heI] - houses[hoI]);
      if (heI > 0) {
        distance = min(distance, abs(heaters[heI - 1] - houses[hoI]));
      }
